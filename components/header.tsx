...
>
  ...
  >
    ...
    {/* Content Wrapper (Grows to push buttons down) */}
        ...
        !link.sublinks ? (
          ...
          <div key={link.name} className="flex flex-col cursor-pointer gap-5">
            <button
              ...
              className={`flex items-center justify-between cursor-pointer text-sm font-medium ${textDark} hover:text-[${primary}]`}
            >
              {link.name}
              <ChevronDown
    ...
    {/* Auth Buttons Section (Anchored perfectly to the bottom) */}
    <div className="flex flex-col gap-4 mt-auto pt-8 pb-4">
      <Link href="/auth/login" onClick={() => setIsOpen(false)}>
        <Button
          className={`bg-[${primary}] text-white hover:bg-[${primaryDarker}] w-full py-5 rounded-md text-sm`}
        >
          Login Your Account
        </Button>
      </Link>
      <Link href="/auth/register" onClick={() => setIsOpen(false)}>
        <Button
          variant="outline"
          className={`border border-[${primary}] text-[${primary}] hover:bg-[${primary}]/10 dark:hover:bg-[${primary}]/[0.05] w-full py-5 rounded-md text-sm`}
        >
          Register an Account
        </Button>
      </Link>

...
export async function loginUser(email, password) {
  try {
    ...
    // console.log('Login token:', data.token);
      ...
      // Store token in cookie as 'auth-token' (without Secure flag for localhost)
      const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
      document.cookie = `auth-token=${data.token}; Path=/; ${isLocalhost ? '' : 'Secure;'} SameSite=Lax;`;
    }
...
function getAuthToken() {
  // Try cookie first (works in production with same-domain)
  const cookieToken = getCookie('auth-token');
  if (cookieToken) return cookieToken;
  
  // Fallback to localStorage (works in local development with cross-origin)
  if (typeof window !== 'undefined') {
    return localStorage.getItem('cy_token');
  }

...
import {
  ...
  Search,
  Filter,
  // Loader2 ← removed from import (no longer used)
} from "lucide-react";
...
import { FeatureFlagsSkeleton } from "@/components/ui/FeatureFlagsSkeleton";
...
const FeatureFlagsPage: React.FC = () => {
  ...
  const [isLoading, setIsLoading] = useState(true); // kept only for disabling inputs while fetching
  ...
  const handleToggle = async (flagId: string, currentState: boolean) => {
    try {
      ...
      toast.success(
        `Feature flag ${newState ? "enabled" : "disabled"} successfully`
      );
    } catch (err) {

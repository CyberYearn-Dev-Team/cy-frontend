...
const router = useRouter();
  ...
  // Passing score — you can decide whether to use current quiz or average
  const passingScore = currentQuiz?.passing_score ?? 0;
  const passed = score >= passingScore;

  // Example XP calculation: 10 XP per correct answer
  const xp = correctCount * 10;

  // Redirect with results
 router.push(
  `/learner-dashboard/tracks/${slug}/modules/${moduleSlug}/lessons/${lessonSlug}/quizzes/results?score=${score}&passed=${passed}&xp=${xp}&results=${encodeURIComponent(
...
<div className="flex justify-between items-center">
  ...
  {currentFlatIndex === totalQuestions && !submitted ? (
    <button
      ...
      className="px-4 py-2 rounded bg-[#72a210] text-white hover:bg-[#5c880d]"
    >
      Submit Answers
    </button>
  ) : (
    <button
      ...
      className="px-4 py-2 rounded bg-[#72a210] text-white hover:bg-[#5c880d] disabled:opacity-50"
    >
      Next
    </button>
  )}
